...
export async function fetchPricingConfig() {
    ...
    try {
        const { data, error } = await supabaseClient
            .from('pricing_config')
            .select('steam_fee_percent, steam_min_amount, stars_price_per_unit, stars_min_qty')
            .eq('id', 1)
            .maybeSingle();
...
export function applyPricingConfig(catalog, config) {
    ...
    if (steam) {
        ...
        if (steam.quickAmounts) steam.quickAmounts = steam.quickAmounts.map((v) => Math.max(v, steam.minAmount));
    }
    ...
    if (stars) {
        ...
        if (stars.quickAmounts) stars.quickAmounts = stars.quickAmounts.map((v) => Math.max(v, stars.minQty));
    }
}
