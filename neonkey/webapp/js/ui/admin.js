...
export async function renderAdminOrders() {
    ...
    container.querySelectorAll('[data-action="complete"]').forEach((btn) => {
        btn.addEventListener('click', async function () {
            ...
            if (!confirmed) return;
            const success = await updateOrderStatus(this.dataset.orderId, 'completed');
            if (success) renderAdminOrders();
            else tg.showAlert('Не удалось обновить статус');
        });
    });
    ...
    container.querySelectorAll('[data-action="cancel"]').forEach((btn) => {
        btn.addEventListener('click', async function () {
            ...
            if (!confirmed) return;
            const success = await updateOrderStatus(this.dataset.orderId, 'canceled');
            if (success) renderAdminOrders();
            else tg.showAlert('Не удалось обновить статус');
        });
    });
