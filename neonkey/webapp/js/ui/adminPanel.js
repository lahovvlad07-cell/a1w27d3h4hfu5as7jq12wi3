...
export function initAdminPanel() {
    ...
    const steamMaxInput = document.getElementById('adminSteamMax');
    ...
    const starsMaxInput = document.getElementById('adminStarsMax');
    ...
    steamSaveBtn.addEventListener('click', async () => {
        ...
        const maxAmount = Number(steamMaxInput.value);
        const valid = Number.isFinite(feePercent) && feePercent >= 0
            && Number.isFinite(minAmount) && minAmount > 0
            && Number.isFinite(maxAmount) && maxAmount > 0;
        if (!valid) {
            showToast('Проверьте значения — комиссия, минимум и максимум должны быть положительными числами', 'error');
            return;
        }
        if (maxAmount < minAmount) {
            showToast('Максимальная сумма не может быть меньше минимальной', 'error');
            return;
        }

        const { error } = await updatePricingConfig({
            steam_fee_percent: feePercent,
            steam_min_amount: minAmount,
            steam_max_amount: maxAmount,
        });
        ...
        if (steam) {
            ...
            steam.maxAmount = maxAmount;
            if (steam.defaultAmount < minAmount) steam.defaultAmount = minAmount;
            if (steam.defaultAmount > maxAmount) steam.defaultAmount = maxAmount;
            if (steam.quickAmounts) steam.quickAmounts = steam.quickAmounts.map((v) => Math.min(maxAmount, Math.max(v, minAmount)));
        }
        renderCatalog();
    ...
    starsSaveBtn.addEventListener('click', async () => {
        ...
        const maxQty = Number(starsMaxInput.value);
        const valid = Number.isFinite(pricePerUnit) && pricePerUnit > 0
            && Number.isFinite(minQty) && minQty > 0
            && Number.isFinite(maxQty) && maxQty > 0;
        if (!valid) {
            showToast('Проверьте значения — цена, минимум и максимум должны быть положительными числами', 'error');
            return;
        }
        if (maxQty < minQty) {
            showToast('Максимальное количество не может быть меньше минимального', 'error');
            return;
        }

        const { error } = await updatePricingConfig({
            stars_price_per_unit: pricePerUnit,
            stars_min_qty: minQty,
            stars_max_qty: maxQty,
        });
        if (error) { showToast(`Не удалось сохранить: ${error}`, 'error'); return; }
        ...
        if (stars) {
            ...
            if (stars.defaultQty > maxQty) stars.defaultQty = maxQty;
            if (stars.quickAmounts) stars.quickAmounts = stars.quickAmounts.map((v) => Math.min(maxQty, Math.max(v, minQty)));
        }
        renderCatalog();
