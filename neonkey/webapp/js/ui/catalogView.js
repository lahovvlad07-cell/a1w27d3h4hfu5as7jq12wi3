...
/** Каталог виден всем (для SEO и понимания ассортимента), но покупка
 *  доступна только после входа — это решает setCatalogLocked(). */
export function setCatalogLocked(isLocked) {
    document.getElementById('catalogWrap')?.classList.toggle('is-locked', isLocked);
}

export function initCatalogButtons(checkoutModal, openAuth) {
    const grid = document.getElementById('shopGrid');
    if (!grid) return;
