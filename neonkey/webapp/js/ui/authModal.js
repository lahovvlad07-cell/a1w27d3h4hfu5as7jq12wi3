...
import { renderTelegramLoginWidget, signInWithTelegram, linkTelegramAccount } from '../lib/telegramAuth.js';
import { signInWithEmail, signUpWithEmail } from '../lib/auth.js';
import { supabaseClient } from '../lib/supabaseClient.js';
import { state } from '../state.js';
import { showToast } from './toast.js';

export function initAuthModal({ onAuthed, onLinked }) {
        ...
        if (!widgetRendered) {
            renderTelegramLoginWidget('authTelegramWidget', {
                onAuth: async (telegramUser) => {
                    // Критично: если человек уже вошёл (например, по email), подтверждение
                    // через виджет Telegram НЕ должно переключать его на другой аккаунт —
                    // иначе получится ровно тот баг, из-за которого у пользователя "менялись"
                    // email и аватар: Telegram-виджет молча логинил его в другой, отдельный
                    // аккаунт (созданный под служебный telegram-email), а не привязывался к
                    // текущему. Поэтому при активной сессии всегда только привязываем.
                    if (state.user) {
                        ...
                        close();
                        showToast('Telegram привязан к вашему аккаунту', 'success');
                        onLinked?.(state.user);
                        return;
                    }
