// =========================================================
// NEONKEY — ЛИЧНЫЙ КАБИНЕТ (account.html)
// Вся страница требует входа: без сессии показывается только экран
// «Войдите, чтобы продолжить» с той же модалкой входа, что на лендинге.
// После входа — три вкладки: Каталог, Профиль, Информация.
// =========================================================
import { CHANNEL_URL, SUPPORT_URL } from './config.js';
import { supabaseClient } from './lib/supabaseClient.js';
import { state } from './state.js';
import { CATALOG } from './data/catalog.js';
import { fetchPricingConfig, applyPricingConfig, isAdminUser } from './lib/pricingConfig.js';
import { renderCatalog, initCatalogButtons } from './ui/catalogView.js';
import { initCheckoutModal } from './ui/checkoutModal.js';
import { initBuyModal } from './ui/buyModal.js';
import { initAuthModal } from './ui/authModal.js';
import { initAvatarPicker } from './ui/avatarPicker.js';
import { initTelegramLinkModal } from './ui/telegramLinkModal.js';
import { initProfilePage } from './ui/profilePage.js';
import { initInfoTabs } from './ui/infoTabs.js';
import { initAdminPanel } from './ui/adminPanel.js';
import { showToast } from './ui/toast.js';
import { initScrollReveal } from './lib/scrollReveal.js';

document.querySelectorAll('.js-channel-link').forEach((el) => el.addEventListener('click', (e) => { e.preventDefault(); window.open(CHANNEL_URL, '_blank', 'noopener'); }));
document.querySelectorAll('.js-support-link').forEach((el) => el.addEventListener('click', (e) => { e.preventDefault(); window.open(SUPPORT_URL, '_blank', 'noopener'); }));

const gateEl = document.getElementById('accountGate');
const shellEl = document.getElementById('accountShell');
let profilePageRef = null;
let adminPanelRef = null;

// ---------- Вкладки кабинета ----------
function initTabs(extraTabs = []) {
    const tabs = document.querySelectorAll('.account-tab');
    const panels = document.querySelectorAll('.account-panel');
    const allowed = ['catalog', 'profile', 'info', ...extraTabs];

    function activate(name) {
        if (!allowed.includes(name)) name = 'catalog';
        tabs.forEach((t) => t.classList.toggle('is-active', t.dataset.accountTab === name));
        panels.forEach((p) => p.classList.toggle('is-active', p.id === `panel-${name}`));
    }

    tabs.forEach((tab) => tab.addEventListener('click', () => {
        activate(tab.dataset.accountTab);
        history.replaceState(null, '', `#${tab.dataset.accountTab}`);
    }));

    const initial = location.hash.replace('#', '') || 'catalog';
    activate(initial);
}

// ---------- Инициализация содержимого кабинета (один раз, при первом входе) ----------
let contentInitialized = false;
async function initAccountContent() {
    if (contentInitialized) return;
    contentInitialized = true;

    // Дефолтные цены из data/catalog.js подменяются значениями из Supabase,
    // если админ (@nellmet) уже что-то поменял в админке — см. pricingConfig.js.
    const pricingConfig = await fetchPricingConfig();
    applyPricingConfig(CATALOG, pricingConfig);

    const checkoutModal = initCheckoutModal();
    const buyModal = initBuyModal({ checkoutModal });
    renderCatalog();
    initCatalogButtons(buyModal);

    const adminPanel = initAdminPanel();
    adminPanel.setVisible(isAdminUser(state.user));
    adminPanelRef = adminPanel;

    const avatarPicker = initAvatarPicker({ onSaved: () => profilePage.render() });
    const telegramLinkModal = initTelegramLinkModal({
        onLinked: async () => {
            // Привязка обновляет данные пользователя на сервере через
            // service_role, но текущая сессия в браузере — это токен,
            // выданный ДО привязки, и в нём ещё нет telegram_username. Одного
            // getUser() (просто прочитать свежие данные) недостаточно — сам
            // сохранённый токен в localStorage останется старым, и после
            // перехода на другую страницу/обновления Telegram снова
            // "пропадёт". refreshSession() выпускает новый токен и
            // перезаписывает сохранённую сессию — так изменения переживают
            // перезагрузку и переходы между страницами.
            if (supabaseClient) {
                const { data } = await supabaseClient.auth.refreshSession();
                if (data?.user) state.user = data.user;
            }
            profilePage.render();
            adminPanel.setVisible(isAdminUser(state.user));
        },
    });
    const profilePage = initProfilePage({
        avatarPicker,
        telegramLinkModal,
        onSignedOut: () => {
            state.user = null;
            showGate();
            showToast('Вы вышли из аккаунта', 'info');
        },
    });
    profilePage.render();
    profilePageRef = profilePage;

    initInfoTabs();
    initTabs(isAdminUser(state.user) ? ['admin'] : []);
    initScrollReveal();
}

function showGate() {
    gateEl.classList.remove('hidden');
    shellEl.classList.add('hidden');
}
function showShell() {
    gateEl.classList.add('hidden');
    shellEl.classList.remove('hidden');
    initAccountContent();
}

// ---------- Модалка входа (доступна и на этой странице — прямо на гейте) ----------
const authModal = initAuthModal({
    onAuthed: (user) => {
        state.user = user;
        showShell();
        showToast('Добро пожаловать в личный кабинет', 'success');
    },
    onLinked: () => {
        // Сработает, только если Telegram подтвердили, уже будучи внутри
        // кабинета — на всякий случай синхронизируем вкладку «Профиль».
        profilePageRef?.render();
        adminPanelRef?.setVisible(isAdminUser(state.user));
    },
});
void authModal;

// ---------- Проверка сессии при загрузке ----------
if (supabaseClient) {
    supabaseClient.auth.getSession().then(({ data }) => {
        if (data?.session?.user) {
            state.user = data.session.user;
            showShell();
        } else {
            showGate();
        }
    });
} else {
    showGate();
}
