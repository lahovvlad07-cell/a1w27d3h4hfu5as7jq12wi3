// ===== EDGE FUNCTION: ПРИВЯЗКА EMAIL К ТЕКУЩЕМУ АККАУНТУ (переподвязка) =====
// Используется кнопкой "Привязать email" в профиле у пользователей,
// вошедших через Telegram (у которых вместо настоящего email —
// служебный tg-<id>@telegram.neonkey.app, см. _shared/telegram.ts).
//
...
// (именно с этой ошибкой столкнулся пользователь).
//
...
// подтверждённым — иначе пришлось бы городить отдельный флоу
// подтверждения через письмо только на новый адрес.
//
// Компромисс: мы НЕ проверяем, что пользователь действительно владеет
// email, который вводит (нет письма-подтверждения). Если это важно —
// дальше можно добавить подтверждение через admin.generateLink() +
// собственную отправку письма, но для "просто привязать почту к
// Telegram-аккаунту" это обычно достаточно.
